...
- (void)setListAdapter:(IGListAdapter *)listAdapter
{
   ...
   * It's unfortunate that we need to do this, but once https://github.com/Instagram/IGListKit/pull/375
   * and https://github.com/Instagram/IGListKit/pull/379 land we'll be able to do it via public methods.
   */
  @try {
    if (id updater = [listAdapter valueForKey:@"updatingDelegate"]) {
      if ([updater isKindOfClass:[IGListAdapterUpdater class]]) {
        [updater setValue:@NO forKey:@"canBackgroundReload"];
      } else {
    ...
    ASDisplayNodeFailAssert(@"This version of AsyncDisplayKit is not compatible with this version of IGListKit.");
  }
